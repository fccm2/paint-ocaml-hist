...
module Lst = struct
  let iter_appnd f d ps mg =
    let rec aux_iter ps mg =
      match ps with
      | p :: ps -> aux_iter ps (f (p) d mg)
      | [] -> mg
    in
    aux_iter ps mg
...
let add_ln_bk (ax, ay) (bx, by) d mg =
  ...
  *)
  let ps0 = new_ln_h (x0, x1) y0 in
  let ps4 = new_ln_v x1 (y0, y1) in
  let ps1 = new_ln_h (x1, x2) y1 in

  let mg = Lst.iter_appnd (add_pnt) d ps0 mg in
  let mg = Lst.iter_appnd (add_pnt) d ps4 mg in
  let mg = Lst.iter_appnd (add_pnt) d ps1 mg in

  (mg)


let strk_rct (x, y, w, h) d mg =
  let ax, ay = (x, y) in
  let bx, by = (x+w, y+h) in
  ...
  *)
  ...
  let ps0 = new_ln_v x0 (y0, y1) in
  let ps1 = new_ln_v x1 (y0, y1) in

  let ps5 = new_ln_h (x0, x1) y0 in
  let ps4 = new_ln_h (x0, x1) y1 in

  let mg = Lst.iter_appnd (add_pnt) d ps0 mg in
  let mg = Lst.iter_appnd (add_pnt) d ps4 mg in
  let mg = Lst.iter_appnd (add_pnt) d ps1 mg in
  let mg = Lst.iter_appnd (add_pnt) d ps5 mg in
...
val add_ln : int * int -> int * int -> int -> mg -> mg
*)


let add_ln (x0, y0) (x1, y1) d mg =
  let ps = new_ln (x0, y0) (x1, y1) in

  let rec aux_iter ps mg =
    match ps with
    | p :: ps -> aux_iter ps (add_pnt (p) d mg)
    | [] -> mg
  in
  aux_iter ps mg
